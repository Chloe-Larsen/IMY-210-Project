...
app.get('/transform', async (req, res) => {//Semester 1
    try {
        ...
        const xmlPath = path.join(storageDir, xml);
        const xsltPath = path.join(storageDir, xslt);
        ...
        const htmlOutput = await xsltProcessor.xsltProcess(xmlDoc, xsltDoc);
...
app.get('/generate-pdf', async (req, res) => {
    ...
    const outputName = xml.replace('.xml', '.pdf');
